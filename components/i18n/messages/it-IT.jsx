import { LOCALES } from "../locales";

export default {
  [LOCALES.ITALIAN]: {
    hello: "Hello",
    25: "25",
    50: "50",
    100: "100",
    500: "500",
    1000: "1000",
    2500: "2500",
    donate_now: "DONA ORA",
    amount: "Importo",
    billing: "Fatturazione",
    payment: "Pagamento",
    make_contribution_monthly: "Fai questo contributo mensilmente",
    make_contribution_in_honor:
      "Vorrei dare questo contributo in onore o in memoria di qualcuno",
    next: "Successivo",
    back: "Indietro",
    first_name: "Nome",
    middle_name: "Secondo nome (facoltativo)",
    last_name: "Cognome",
    address: "Indirizzo",
    city: "Città",
    postal_code: "Codice postale",
    country: "Paese",
    state: "Stato/Provincia",
    province: "Stato/Provincia",
    phone: "Telefono",
    email: "Email",
    summary: "Sommario",
    donation: "Donazioni ",
    url: "Politica di Due Diligence dei Partner",
    certify:
      "Dichiaro che non sto effettuando la presente donazione a nome di nessun partito, organizzazione o individuo che violi la {path}.",
    processing: "Lavorazione...",
    process_amount: "Dona {amount} Ora",
    fund: "Fondo per la democratizzazione dei dati ",
    monthly_donation: "Donazione mensile",
    // honoree form
    in_honor: "In onore di",
    in_memory: "In memoria di",
    // Notifications
    exchange_notification:
      "Riflette il tasso di cambio di {exchange}. Il tasso di conversione dei singoli istituti finanziari può variare.",
    // Errors
    error: "Please enter a valid {e}",
    incorrect_number: "Il numero di carta è errato.",
    invalid_number: "Il numero di carta non è un numero di carta valido.",
    invalid_expiry_month: "Il mese di scadenza della carta non è valido.",
    invalid_expiry_year: "L’anno di scadenza della carta non è valido.",
    invalid_cvc: "Il codice di sicurezza della carta non è valido.",
    expired_card: "La carta è scaduta.",
    incorrect_cvc: "Il codice di sicurezza della carta è errato.",
    incorrect_zip: "Il CAP della carta non è valido.",
    card_declined: "La carta è stata declinata.",
    missing: "La carta di uno dei clienti addebitati è mancante.",
    processing_error: "C’è stato un errore nell’elaborazione della carta.",
    rate_limit:
      "C’è stato un errore in quanto la richiesta ha raggiunto l’API troppo in fretta. Ti preghiamo di comunicarci se questo errore dovesse capitare ripetutamente.",
    // Success Page
    transaction_complete: "Transaction Complete!",
    thank_you:
      "Grazie per il tuo generoso contributo a sostegno degli sforzi di @avecanalytics per #democratizedata",
    email_confirmation:
      "Un’e-mail di conferma sarà inviata all’indirizzo di posta elettronica fornito.",
    twitter_share: "Condividi il tuo sostegno su Twitter!",
    tweet: "Tweet:",
    example_tweet:
      "Ho appena fatto una donazione al fondo #datademocratization per sostenere gli sforzi di @avecanalytics per rispondere a #democratizedata. Unisciti a me!",
    print_confirmation:
      "La preghiamo di stampare questa pagina di conferma del contributo per conservarla tra i Suoi documenti.",
    contribution_information: "Informazioni sul contributo:",
    contribution_confirmation_id: "ID di Conferma del contributo:",
    contribution_date: "Data del contributo:",
    contribution_amount: "Importo del contributo:",
    contact_information: "Informazioni di contatto:",
    account_type: "Tipo di conto:",
    account_number_ending: "Il numero di conto finisce con:",
    receipt: "Scontrino",
    // Countries
    AF: "Afghanistan",
    AL: "Albania",
    DZ: "Algeria",
    UM: "Altre isole americane del Pacifico",
    AD: "Andorra",
    AO: "Angola",
    AI: "Anguilla",
    AQ: "Antartide",
    AG: "Antigua e Barbuda",
    SA: "Arabia Saudita",
    AR: "Argentina",
    AM: "Armenia",
    AW: "Aruba",
    AU: "Australia",
    AT: "Austria",
    AZ: "Azerbaigian",
    BS: "Bahamas",
    BH: "Bahrein",
    BD: "Bangladesh",
    BB: "Barbados",
    BE: "Belgio",
    BZ: "Belize",
    BJ: "Benin",
    BM: "Bermuda",
    BT: "Bhutan",
    BY: "Bielorussia",
    BO: "Bolivia",
    BA: "Bosnia ed Erzegovina",
    BW: "Botswana",
    BR: "Brasile",
    BN: "Brunei",
    BG: "Bulgaria",
    BF: "Burkina Faso",
    BI: "Burundi",
    KH: "Cambogia",
    CM: "Camerun",
    CA: "Canada",
    CV: "Capo Verde",
    BQ: "Caraibi olandesi",
    CZ: "Cechia",
    TD: "Ciad",
    CL: "Cile",
    CN: "Cina",
    CY: "Cipro",
    VA: "Città del Vaticano",
    CO: "Colombia",
    KM: "Comore",
    CD: "Congo - Kinshasa",
    CG: "Congo-Brazzaville",
    KP: "Corea del Nord",
    KR: "Corea del Sud",
    CI: "Costa d’Avorio",
    CR: "Costa Rica",
    HR: "Croazia",
    CU: "Cuba",
    CW: "Curaçao",
    DK: "Danimarca",
    DM: "Dominica",
    EC: "Ecuador",
    EG: "Egitto",
    SV: "El Salvador",
    AE: "Emirati Arabi Uniti",
    ER: "Eritrea",
    EE: "Estonia",
    ET: "Etiopia",
    FJ: "Figi",
    PH: "Filippine",
    FI: "Finlandia",
    FR: "Francia",
    GA: "Gabon",
    GM: "Gambia",
    GE: "Georgia",
    GS: "Georgia del Sud e Sandwich australi",
    DE: "Germania",
    GH: "Ghana",
    JM: "Giamaica",
    JP: "Giappone",
    GI: "Gibilterra",
    DJ: "Gibuti",
    JO: "Giordania",
    GR: "Grecia",
    GD: "Grenada",
    GL: "Groenlandia",
    GP: "Guadalupa",
    GU: "Guam",
    GT: "Guatemala",
    GG: "Guernsey",
    GN: "Guinea",
    GQ: "Guinea Equatoriale",
    GW: "Guinea-Bissau",
    GY: "Guyana",
    GF: "Guyana francese",
    HT: "Haiti",
    HN: "Honduras",
    IN: "India",
    ID: "Indonesia",
    IR: "Iran",
    IQ: "Iraq",
    IE: "Irlanda",
    IS: "Islanda",
    BV: "Isola Bouvet",
    CX: "Isola Christmas",
    IM: "Isola di Man",
    NF: "Isola Norfolk",
    AX: "Isole Åland",
    KY: "Isole Cayman",
    CC: "Isole Cocos (Keeling)",
    CK: "Isole Cook",
    FO: "Isole Fær Øer",
    FK: "Isole Falkland",
    HM: "Isole Heard e McDonald",
    MP: "Isole Marianne settentrionali",
    MH: "Isole Marshall",
    PN: "Isole Pitcairn",
    SB: "Isole Salomone",
    TC: "Isole Turks e Caicos",
    VI: "Isole Vergini Americane",
    VG: "Isole Vergini Britanniche",
    IL: "Israele",
    IT: "Italia",
    JE: "Jersey",
    KZ: "Kazakistan",
    KE: "Kenya",
    KG: "Kirghizistan",
    KI: "Kiribati",
    KW: "Kuwait",
    LA: "Laos",
    LS: "Lesotho",
    LV: "Lettonia",
    LB: "Libano",
    LR: "Liberia",
    LY: "Libia",
    LI: "Liechtenstein",
    LT: "Lituania",
    LU: "Lussemburgo",
    MK: "Macedonia del Nord",
    MG: "Madagascar",
    MW: "Malawi",
    MY: "Malaysia",
    MV: "Maldive",
    ML: "Mali",
    MT: "Malta",
    MA: "Marocco",
    MQ: "Martinica",
    MR: "Mauritania",
    MU: "Mauritius",
    YT: "Mayotte",
    MX: "Messico",
    FM: "Micronesia",
    MD: "Moldavia",
    MC: "Monaco",
    MN: "Mongolia",
    ME: "Montenegro",
    MS: "Montserrat",
    MZ: "Mozambico",
    MM: "Myanmar (Birmania)",
    NA: "Namibia",
    NR: "Nauru",
    NP: "Nepal",
    NI: "Nicaragua",
    NE: "Niger",
    NG: "Nigeria",
    NU: "Niue",
    NO: "Norvegia",
    NC: "Nuova Caledonia",
    NZ: "Nuova Zelanda",
    OM: "Oman",
    NL: "Paesi Bassi",
    PK: "Pakistan",
    PW: "Palau",
    PA: "Panamá",
    PG: "Papua Nuova Guinea",
    PY: "Paraguay",
    PE: "Perù",
    PF: "Polinesia francese",
    PL: "Polonia",
    PT: "Portogallo",
    PR: "Portorico",
    QA: "Qatar",
    HK: "RAS di Hong Kong",
    MO: "RAS di Macao",
    GB: "Regno Unito",
    CF: "Repubblica Centrafricana",
    DO: "Repubblica Dominicana",
    RE: "Riunione",
    RO: "Romania",
    RW: "Ruanda",
    RU: "Russia",
    EH: "Sahara occidentale",
    KN: "Saint Kitts e Nevis",
    LC: "Saint Lucia",
    MF: "Saint Martin",
    VC: "Saint Vincent e Grenadine",
    BL: "Saint-Barthélemy",
    PM: "Saint-Pierre e Miquelon",
    WS: "Samoa",
    AS: "Samoa americane",
    SM: "San Marino",
    SH: "Sant’Elena",
    ST: "São Tomé e Príncipe",
    SN: "Senegal",
    RS: "Serbia",
    SC: "Seychelles",
    SL: "Sierra Leone",
    SG: "Singapore",
    SX: "Sint Maarten",
    SY: "Siria",
    SK: "Slovacchia",
    SI: "Slovenia",
    SO: "Somalia",
    ES: "Spagna",
    LK: "Sri Lanka",
    US: "Stati Uniti",
    SS: "Sud Sudan",
    ZA: "Sudafrica",
    SD: "Sudan",
    SR: "Suriname",
    SJ: "Svalbard e Jan Mayen",
    SE: "Svezia",
    CH: "Svizzera",
    SZ: "Swaziland",
    TJ: "Tagikistan",
    TW: "Taiwan",
    TZ: "Tanzania",
    TF: "Terre australi francesi",
    PS: "Territori palestinesi",
    IO: "Territorio britannico dell’Oceano Indiano",
    TH: "Thailandia",
    TL: "Timor Est",
    TG: "Togo",
    TK: "Tokelau",
    TO: "Tonga",
    TT: "Trinidad e Tobago",
    TN: "Tunisia",
    TR: "Turchia",
    TM: "Turkmenistan",
    TV: "Tuvalu",
    UA: "Ucraina",
    UG: "Uganda",
    HU: "Ungheria",
    UY: "Uruguay",
    UZ: "Uzbekistan",
    VU: "Vanuatu",
    VE: "Venezuela",
    VN: "Vietnam",
    WF: "Wallis e Futuna",
    YE: "Yemen",
    ZM: "Zambia",
    ZW: "Zimbabwe",
    // currencies
    ALK: "Albanian Lek (1946–1965)",
    ARM: "Argentine Peso (1881–1970)",
    ARL: "Argentine Peso Ley (1970–1983)",
    BOL: "Bolivian Boliviano (1863–1963)",
    BAN: "Bosnia-Herzegovina New Dinar (1994–1997)",
    BRZ: "Brazilian Cruzeiro (1942–1967)",
    BGO: "Bulgarian Lev (1879–1952)",
    BGM: "Bulgarian Socialist Lev",
    CLE: "Chilean Escudo",
    CNX: "Chinese People’s Bank Dollar",
    COU: "Colombian Real Value Unit",
    XEU: "European Currency Unit",
    ISJ: "Icelandic Króna (1918–1981)",
    ILR: "Israeli Shekel (1980–1985)",
    MKN: "Macedonian Denar (1992–1993)",
    MVP: "Maldivian Rupee (1947–1981)",
    MDC: "Moldovan Cupon",
    MCF: "Monegasque Franc",
    MZM: "Mozambican Metical (1980–2006)",
    KRH: "South Korean Hwan (1953–1962)",
    KRO: "South Korean Won (1945–1953)",
    SDP: "Sudanese Pound (1957–1998)",
    VNN: "Vietnamese Dong (1978–1985)",
    CHE: "WIR Euro",
    CHW: "WIR Franc",
    YUR: "Yugoslavian Reformed Dinar (1992–1993)",
    ZWR: "Zimbabwean Dollar (2008)",
    AFA: "afgani (1927–2002)",
    AFN: "afghani",
    CSD: "antico dinaro serbo",
    MGA: "ariary malgascio",
    ARA: "austral argentino",
    THB: "baht thailandese",
    PAB: "balboa panamense",
    ETB: "birr etiope",
    VEB: "bolivar venezuelano (1871–2008)",
    BOB: "boliviano",
    VEF: "bolívar venezuelano",
    GHC: "cedi del Ghana",
    GHS: "cedi ghanese",
    CRC: "colón costaricano",
    SVC: "colón salvadoregno",
    NIC: "cordoba nicaraguense",
    CZK: "corona ceca",
    DKK: "corona danese",
    EEK: "corona dell’Estonia",
    CSK: "corona forte cecoslovacca",
    ISK: "corona islandese",
    NOK: "corona norvegese",
    SKK: "corona slovacca",
    SEK: "corona svedese",
    BRC: "cruzado brasiliano",
    BRN: "cruzado novo brasiliano",
    BRR: "cruzeiro brasiliano",
    BRE: "cruzeiro brasiliano (1990–1993)",
    BRB: "cruzeiro novo brasiliano (1967–1986)",
    NIO: "córdoba nicaraguense",
    GMD: "dalasi gambiano",
    MKD: "denar macedone",
    BAD: "dinar Bosnia-Herzegovina",
    DZD: "dinaro algerino",
    YUN: "dinaro convertibile yugoslavo",
    HRD: "dinaro croato",
    BHD: "dinaro del Bahrein",
    YDD: "dinaro dello Yemen",
    YUD: "dinaro forte yugoslavo",
    JOD: "dinaro giordano",
    IQD: "dinaro iracheno",
    KWD: "dinaro kuwaitiano",
    LYD: "dinaro libico",
    YUM: "dinaro noviy yugoslavo",
    RSD: "dinaro serbo",
    SDD: "dinaro sudanese",
    TND: "dinaro tunisino",
    AED: "dirham degli Emirati Arabi Uniti",
    MAD: "dirham marocchino",
    STD: "dobra di Sao Tomé e Principe",
    AUD: "dollaro australiano",
    CAD: "dollaro canadese",
    XCD: "dollaro dei Caraibi orientali",
    BZD: "dollaro del Belize",
    BND: "dollaro del Brunei",
    SRD: "dollaro del Suriname",
    GYD: "dollaro della Guyana",
    RHD: "dollaro della Rhodesia",
    BSD: "dollaro delle Bahamas",
    BMD: "dollaro delle Bermuda",
    FJD: "dollaro delle Figi",
    KYD: "dollaro delle Isole Cayman",
    SBD: "dollaro delle Isole Salomone",
    ZWD: "dollaro dello Zimbabwe",
    BBD: "dollaro di Barbados",
    HKD: "dollaro di Hong Kong",
    SGD: "dollaro di Singapore",
    TTD: "dollaro di Trinidad e Tobago",
    JMD: "dollaro giamaicano",
    LRD: "dollaro liberiano",
    NAD: "dollaro namibiano",
    NZD: "dollaro neozelandese",
    USD: "dollaro statunitense",
    USN: "dollaro statunitense (next day)",
    USS: "dollaro statunitense (same day)",
    ZWL: "dollaro zimbabwiano (2009)",
    VND: "dong vietnamita",
    GRD: "dracma greca",
    AMD: "dram armeno",
    GQE: "ekwele della Guinea Equatoriale",
    CVE: "escudo capoverdiano",
    MZE: "escudo del Mozambico",
    GWE: "escudo della Guinea portoghese",
    TPE: "escudo di Timor",
    PTE: "escudo portoghese",
    EUR: "euro",
    SRG: "fiorino del Suriname",
    ANG: "fiorino delle Antille olandesi",
    AWG: "fiorino di Aruba",
    NLG: "fiorino olandese",
    HUF: "fiorino ungherese",
    XRE: "fondi RINET",
    XOF: "franco CFA BCEAO",
    XAF: "franco CFA BEAC",
    XPF: "franco CFP",
    XFU: "franco UIC francese",
    BEF: "franco belga",
    BEC: "franco belga (convertibile)",
    BEL: "franco belga (finanziario)",
    KMF: "franco comoriano",
    CDF: "franco congolese",
    LUC: "franco convertibile del Lussemburgo",
    BIF: "franco del Burundi",
    LUF: "franco del Lussemburgo",
    GNF: "franco della Guinea",
    DJF: "franco di Gibuti",
    MLF: "franco di Mali",
    LUL: "franco finanziario del Lussemburgo",
    FRF: "franco francese",
    MGF: "franco malgascio",
    MAF: "franco marocchino",
    XFO: "franco oro francese",
    RWF: "franco ruandese",
    CHF: "franco svizzero",
    HTG: "gourde haitiano",
    UAH: "grivnia ucraina",
    PYG: "guaraní paraguayano",
    PEI: "inti peruviano",
    UAK: "karbovanetz ucraino",
    PGK: "kina papuana",
    LAK: "kip laotiano",
    HRK: "kuna croata",
    GEK: "kupon larit georgiano",
    ZMW: "kwacha dello Zambia",
    ZMK: "kwacha dello Zambia (1968–2012)",
    MWK: "kwacha malawiano",
    AOA: "kwanza angolano",
    AOK: "kwanza angolano (1977–1990)",
    AOR: "kwanza reajustado angolano (1995–1999)",
    BUK: "kyat birmano",
    MMK: "kyat di Myanmar",
    GEL: "lari georgiano",
    LVL: "lats lettone",
    ALL: "lek albanese",
    HNL: "lempira honduregna",
    SLL: "leone della Sierra Leone",
    ROL: "leu della Romania",
    MDL: "leu moldavo",
    RON: "leu rumeno",
    BGN: "lev bulgaro",
    BGL: "lev bulgaro (1962–1999)",
    SZL: "lilangeni dello Swaziland",
    ITL: "lira italiana",
    LBP: "lira libanese",
    MTL: "lira maltese",
    SYP: "lira siriana",
    TRY: "lira turca",
    TRL: "lira turca (1922–2005)",
    LTL: "litas lituano",
    LSL: "loti del Lesotho",
    AZN: "manat azero",
    AZM: "manat azero (1993–2006)",
    TMT: "manat turkmeno",
    TMM: "manat turkmeno (1993–2009)",
    BAM: "marco convertibile della Bosnia-Herzegovina",
    DEM: "marco tedesco",
    FIM: "markka finlandese",
    MZN: "metical mozambicano",
    BOV: "mvdol boliviano",
    NGN: "naira nigeriana",
    ERN: "nakfa eritreo",
    BTN: "ngultrum bhutanese",
    TWD: "nuovo dollaro taiwanese",
    AON: "nuovo kwanza angolano (1990–2000)",
    BYB: "nuovo rublo bielorusso (1994–1999)",
    ILS: "nuovo siclo israeliano",
    ZRN: "nuovo zaire dello Zaire",
    DDM: "ostmark della Germania Orientale",
    MRO: "ouguiya della Mauritania",
    MOP: "pataca di Macao",
    TOP: "paʻanga tongano",
    ADP: "peseta andorrana",
    ESP: "peseta spagnola",
    ESA: "peseta spagnola account",
    ESB: "peseta spagnola account convertibile",
    ARS: "peso argentino",
    ARP: "peso argentino (vecchio Cod.)",
    BOP: "peso boliviano",
    CLP: "peso cileno",
    COP: "peso colombiano",
    CUP: "peso cubano",
    CUC: "peso cubano convertibile",
    GWP: "peso della Guinea-Bissau",
    DOP: "peso dominicano",
    PHP: "peso filippino",
    MXN: "peso messicano",
    MXP: "peso messicano d’argento (1861–1992)",
    UYP: "peso uruguaiano (1975–1993)",
    UYI: "peso uruguaiano in unità indicizzate",
    UYU: "peso uruguayano",
    BWP: "pula del Botswana",
    GTQ: "quetzal guatemalteco",
    ZAR: "rand sudafricano",
    ZAL: "rand sudafricano (finanziario)",
    BRL: "real brasiliano",
    CNY: "renminbi cinese",
    IRR: "rial iraniano",
    OMR: "rial omanita",
    QAR: "rial qatariano",
    KHR: "riel cambogiano",
    MYR: "ringgit malese",
    SAR: "riyal saudita",
    YER: "riyal yemenita",
    BYN: "rublo bielorusso",
    BYR: "rublo bielorusso (2000–2016)",
    TJR: "rublo del Tajikistan",
    RUR: "rublo della CSI",
    LVR: "rublo lettone",
    RUB: "rublo russo",
    SUR: "rublo sovietico",
    MVR: "rufiyaa delle Maldive",
    SCR: "rupia delle Seychelles",
    LKR: "rupia di Sri Lanka",
    INR: "rupia indiana",
    IDR: "rupia indonesiana",
    MUR: "rupia mauriziana",
    NPR: "rupia nepalese",
    PKR: "rupia pakistana",
    ATS: "scellino austriaco",
    TZS: "scellino della Tanzania",
    KES: "scellino keniota",
    SOS: "scellino somalo",
    UGX: "scellino ugandese",
    UGS: "scellino ugandese (1966–1987)",
    PEN: "sol peruviano",
    PES: "sol peruviano (1863–1965)",
    KGS: "som kirghiso",
    TJS: "somoni tagiko",
    GBP: "sterlina britannica",
    CYP: "sterlina cipriota",
    FKP: "sterlina delle Falkland",
    GIP: "sterlina di Gibilterra",
    SHP: "sterlina di Sant’Elena",
    EGP: "sterlina egiziana",
    IEP: "sterlina irlandese",
    ILP: "sterlina israeliana",
    MTP: "sterlina maltese",
    SSP: "sterlina sud-sudanese",
    SDG: "sterlina sudanese",
    ECS: "sucre dell’Ecuador",
    UZS: "sum uzbeco",
    GNS: "syli della Guinea",
    BDT: "taka bangladese",
    WST: "tala samoano",
    SIT: "tallero sloveno",
    LTT: "talonas lituani",
    KZT: "tenge kazako",
    MNT: "tugrik mongolo",
    MXV: "unidad de inversion (UDI) messicana",
    ECV: "unidad de valor constante (UVC) dell’Ecuador",
    CLF: "unidades de fomento chilene",
    VUV: "vatu di Vanuatu",
    KPW: "won nordcoreano",
    KRW: "won sudcoreano",
    JPY: "yen giapponese",
    ZRZ: "zaire dello Zaire",
    PLZ: "złoty Polacco (1950–1995)",
    PLN: "złoty polacco",
  },
};
